...
import { ORG_IMG_BASE_URL, SMALL_IMG_BASE_URL } from "../utils/baseUrl";
      ...
      <Navbar />
        {trailers.length > 0 && (
          <div className="flex justify-between items-center mb-4">
            ...
            >
              <ChevronLeft size={24} onClick={handlePrevios}/>
            </button>
            <button
              className={`bg-gray-500/70 hover:bg-gray-500 tetx-white py-2 px-4 rounded ${
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-20 max-w-6xl mx-auto">
          <div className="mb-4 text-lg">
            ...
            <p className="mt-4 yext-lg">{content?.overview}</p>
          </div>
          <img src={ORG_IMG_BASE_URL + content.poster_path} className="max-h-[600px] rounded-md" alt="" />
        </div>
        {similarContent.length > 0 && (
          <div className="mt-12 max-w-5xl mx-auto relative">
            ...
            <div className="flex overflow-x-scroll scrollbar-hide gap-4 group">
              {similarContent.map((content)=>{
                if(!content.poster_path) return null;
                return (
                <Link key={content.id} to ={`/watch/${content.id}`} className="w-52 flex-none">
                   <img src={SMALL_IMG_BASE_URL + content.poster_path} className="w-full h-full rounded-md" alt="" />
